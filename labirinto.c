...
void copiar(labirinto *l, labirinto *caminho){
    for (int i = 0; i < (*l).linhas; i++) {
        for (int j = 0; j < (*l).colunas; j++) {
            (*caminho).matriz[i][j] = (*l).matriz[i][j];
}}}
...
int main (){
    ...
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            if ((*l).matriz[i][j] == 'P') {
                labirinto *caminho = criar_labirinto(linhas, colunas);

                for (int i = 0; i < (*l).linhas; i++) {
                    for (int j = 0; j < (*l).colunas; j++) {
                        (*caminho).matriz[i][j] = (*l).matriz[i][j];
                }}
                caminho= buscar_saida(caminho, i, j);
                imprimir_labirinto(caminho);
                destruir_labirinto(caminho);
