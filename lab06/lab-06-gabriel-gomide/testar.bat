...
IF NOT EXIST gerenciador.c (
    @ECHO Arquivo gerenciador.c nao encontrado
    PAUSE
    EXIT /B
)
IF NOT EXIST gerenciador.h (
    @ECHO Arquivo gerenciador.h nao encontrado
    PAUSE
    EXIT /B
)
...
IF EXIST TESTES\10.txt DEL /F TESTES\10.txt
...
cliente.bin < TESTES\10.in >> TESTES\10.txt
...
FC TESTES\10.txt TESTES\10.out >> TESTES\Resultado.txt
